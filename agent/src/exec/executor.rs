...
impl ActionExecutor {
    ...
    pub async fn execute(&self, action: &ActionType, pid: u32) -> Result<String, String> {
        match action {
            ActionType::Signal { signal } => {
                self.send_signal(*signal, pid).await
            }
            ActionType::CgroupThrottle { cpu_quota, memory_limit, io_limit } => {
                self.apply_cgroup_throttle(pid, *cpu_quota, *memory_limit, *io_limit).await
            }
            ActionType::NetworkRestart { interface } => {
                self.restart_network_interface(interface).await
            }
            ActionType::GracefulShutdown { signal, wait_seconds, force_kill } => {
                self.graceful_shutdown(*signal, *wait_seconds, *force_kill, pid).await
            }
            ActionType::KillProcess => {
                self.kill_process(pid).await
            }
            ActionType::IsolateNode { reason } => {
                self.isolate_node(reason).await
            }
    ...
    async fn graceful_shutdown(
        ...
        if force_kill {
            match self.kill_process(pid).await {
                Ok(msg) => Ok(format!("优雅降级完成: 已发送信号 {}，等待 {} 秒后{}", 
    ...
    async fn check_checkpoint(&self, checkpoint_dir: &str) -> Result<String, String> {
        ...
        match fs::metadata(checkpoint_dir).await {
            Ok(metadata) => {
                if metadata.is_dir() {
                    let entries = fs::read_dir(checkpoint_dir).await
                    ...
                    while let Ok(Some(entry)) = entries.next_entry().await {
                        count += 1;
                        if let Ok(modified) = entry.metadata().await.and_then(|m| {
                            m.modified().ok_or(std::io::Error::new(
                    ...
                    Ok(format!("Checkpoint 检查完成: 找到 {} 个文件，最新: {:?}", 
                             count, latest))
                } else {
    ...
    /// 执行自定义命令
    async fn execute_custom_command(&self, command: &str, args: &[String]) -> Result<String, String> {
